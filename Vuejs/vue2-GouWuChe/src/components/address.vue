      ...
      <div class="checkout-addr">
            ...
            <ul>
              <li v-for="(item, index) in filterAddress" :class="{'check': index==currentIndex}" @click="currentIndex=index">
                ...
                <div class="addr-opration addr-set-default" v-if="!item.isDefault">
                  <a href="javascript:void(0);" class="addr-set-default-btn" @click="setDefault(item.addressId)"><i>设为默认</i></a>
                </div>
        ...
        <div class="shipping-method-wrap">
          <div class="shipping-method">
            <ul>
              <li :class="{ 'check': isActive==1 }" @click="isActive=1">
                <div class="name">标准配送</div>
                <div class="price">Free</div>
              </li>
              <li :class="{ 'check': isActive==2 }" @click="isActive=2">
	...
	export default{
		...
		methods: {
			getAddressList: function(){
				this.$http.get("http://localhost:3000/address").then(res => {
					var res = res.data;
